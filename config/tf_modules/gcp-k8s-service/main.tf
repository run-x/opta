...
resource "helm_release" "k8s-service" {
  ...
  values = [
    yamlencode({
      ...
      image : var.image == "AUTO" ? (var.digest != null ? "${data.google_container_registry_repository.root.repository_url}/${var.layer_name}/${var.module_name}@${var.digest}" : (var.tag == null ? "" : "${data.google_container_registry_repository.root.repository_url}/${var.layer_name}/${var.module_name}:${var.tag}")) : (var.tag == null ? var.image : "${var.image}:${var.tag}")
