...
class TestLocalK8sServiceProcessor:
    ...
    def test_prehook(self, mocker: MockFixture):
        ...
        mocked_create_namespace_if_not_exists = mocker.patch(
            "opta.module_processors.local_k8s_service.create_namespace_if_not_exists"
        )
        mocked_get_manual_secrets = mocker.patch(
            "opta.module_processors.local_k8s_service.get_manual_secrets"
        )
        mocked_get_manual_secrets.return_value = {"BALONEY": "blah"}
        LocalK8sServiceProcessor(app_module, layer).pre_hook(idx)
