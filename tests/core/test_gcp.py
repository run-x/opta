...
class TestGcp:
    ...
    def test_get_config_map_configuration_present(self, mocker: MockFixture):
        ...
        mock_gcs_client_instance = mocker.Mock(spec=Client)
        mock_gcs_bucket_instance = mocker.Mock(spec=Bucket)
        mock_gcs_bucket_instance.name = "test"
        mock_gcs_blob_instance = mocker.Mock(spec=Blob)
        mock_gcs_blob_instance.name = "opta_config/test-config"
        mock_gcs_client_instance.list_buckets.return_value = [mock_gcs_bucket_instance]
        mock_gcs_client_instance.list_blobs.return_value = [mock_gcs_blob_instance]
        mocker.patch(
            "opta.core.gcp.storage.Client", return_value=mock_gcs_client_instance
        )
        GCP.get_config_map()
        mock_gcs_client_instance.list_blobs.assert_called_once_with(
    ...
    def test_get_config_map_configuration_not_present(self, mocker: MockFixture):
        ...
        mock_gcs_bucket_instance.name = "test"
        mock_gcs_client_instance.list_buckets.return_value = [mock_gcs_bucket_instance]
        mock_gcs_client_instance.list_blobs.return_value = []
        mocker.patch(
            "opta.core.gcp.storage.Client", return_value=mock_gcs_client_instance
        )
        GCP.get_config_map()
    ...
    def test_get_config_map_bucket_not_present(self, mocker: MockFixture):
        ...
        mock_gcs_client_instance = mocker.Mock(spec=Client)
        mock_gcs_client_instance.list_buckets.return_value = []
        mocker.patch(
            "opta.core.gcp.storage.Client", return_value=mock_gcs_client_instance
        )
        GCP.get_config_map()
        mock_gcs_client_instance.list_blobs.assert_not_called()
